...
export default function APIIntegrationExample() {
  ...
  const { data: comments, isLoading: commentsLoading } = useProposalComments(selectedProposalId || 0);
  ...
  return (
    <div className="p-8 space-y-8">
      ...
      <section className="space-y-4">
        ...
        {proposalsLoading && <p className="text-gray-500">Carregando propostas...</p>}
        {proposalsError && <p className="text-red-500">Erro ao carregar propostas</p>}
      ...
      {selectedProposalId && (
        <section className="space-y-4">
          ...
          {commentsLoading && <p className="text-gray-500">Carregando comentários...</p>}
          
          {comments && comments.length > 0 ? (
            <div className="space-y-4">
              {comments.map((comment) => (
                <div key={comment.id} className="border rounded-lg p-4 bg-gray-50">
                  <p className="font-semibold">{comment.user?.name || 'Usuário'}</p>
...
mutate({
  ...
  onError: (error) => console.error(error)
        ...
        <div>
          <h3 className="font-semibold text-lg mb-2">3. Listar Comentários de uma Proposta:</h3>
          <pre className="bg-gray-800 text-gray-100 p-3 rounded overflow-x-auto text-sm">
